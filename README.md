...
**Wrap Over Objects**
여기에 등록한 오브젝트의 지오메트리가 충돌 대상에 추가됩니다. 다른 RibbonWrap을 등록하면 이 리본이 그 리본 위를 타고 넘어가서 서로 관통하지 않습니다. 임의 메시(스트랩, 부착물 등)도 등록 가능하며, 등록된 오브젝트(또는 그 상위 계층)가 움직이면 감김이 실시간으로 갱신됩니다. 다른 RibbonWrap을 등록하면 이 리본이 그 리본 위를 타고 넘어가서 서로 관통하지 않습니다. 임의 메시(스트랩, 부착물 등)도 등록 가능.
